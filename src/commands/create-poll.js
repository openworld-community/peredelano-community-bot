...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((option) =>
      option.setName("mention").setDescription("Нужно ли упоминать @everyone")
    )
    .addBooleanOption((option) =>
      option
        .setName("thread")
  ...
  async execute(interaction) {
    ...
    const answers = Array.from(
      new Set(input.split(";").map((item) => item.trim()))
    );
    ...
    const embed = new EmbedBuilder()
      .setAuthor({
        name: `Опрос от ${interaction.user.username}`,
    ...
    switch (type) {
      ...
      case dropdownType:
        const options = answers.map((answer, i) =>
          new StringSelectMenuOptionBuilder().setLabel(answer).setValue(`${i}`)
        );
    ...
    await interaction.reply({
      content: "Опрос был успешно создан.",
      ephemeral: true,
    });
  },
...
async function closeExpiredPolls() {
  const expiredPolls = await sequelize.query(
    `select * from polls where not is_expired and expired_at < Datetime();`,
    {
